...
class BOPLoanRepayment(Document):
	...
	def update_bop_loan(name, amount_paid):
		...
		loan_name = loan.name
		if round(float(amount_paid),2) > round(float(amount),2):
		...
		frappe.db.sql(f"""UPDATE `tabBOP Loan` SET total_principal_paid = {paid_princiapl}, principal_balance = {blnc_principal}, balance_markup_amount = {blnc_markup}, total_markup_paid = {paid_markup} WHERE name= '{loan_name}' """,as_dict=True)
		frappe.db.commit()
		name.save()
	...
	def revert_bop_loan_update(name):
		...
		paid_princiapl = loan.total_principal_amount - blnc_principal
		paid_markup = loan.total_markup_amount + loan.opening_balance - blnc_markup
		frappe.db.sql(f"""UPDATE `tabBOP Loan` SET total_principal_paid = {paid_princiapl}, principal_balance = {blnc_principal}, balance_markup_amount = {blnc_markup}, total_markup_paid = {paid_markup} WHERE name = '{loan_name}' """,as_dict=True)
		frappe.db.commit()
		name.save()
		return "BOP Loan updated successfully."
